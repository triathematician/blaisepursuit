...
public class Tasking {
    ...
    public static final String[] TASKING_STRINGS = {"None", "Autonomous Closest", "Autonomous Two Closest", "Autonomous Farthest",
     ...
     */
    public Tasking(double weight, Vector<Team> teams, Team owner, Team target, int type, int algorithm, double thresh, double exponent) {
        ts = new TaskSettings(weight, teams, owner, target, type, algorithm, thresh, exponent);
        tasker = ts.getTasking();
...
//    }
    ...
    public void setTaskType(int newValue) {
        ts.type.setValue(newValue);
    }

    @XmlAttribute
    public int getAlgorithm() {
        return ts.algorithm.getValue();
    }

    public void setAlgorithm(int newValue) {
        ts.algorithm.setValue(newValue);
    }

    @XmlAttribute
    public double getWeight() {
        return ts.weight.getValue();
    }

    public void setWeight(double newValue) {
        ts.weight.setValue(newValue);
    }

    @XmlAttribute
    public double getThreshhold() {
        return ts.threshhold.getValue();
    }

    public void setThreshhold(double newValue) {
        ts.threshhold.setValue(newValue);
    }

    @XmlAttribute
    public double getExponent() {
        return ts.exponent.getValue();
    }

    public void setExponent(double newValue) {
        ts.exponent.setValue(newValue);
    }

    public String getOwnerName() {
        return ts.owner.getName();
    }
    String targetName = "";

    @XmlAttribute(name = "target")
    public String getTargetName() {
        return ts.target.getName();
    }
    ...
    public class TaskSettings extends Settings {
        ...
        /** Another parameter to use in specifying the goal. Usually the "target" distance required to reach the goal. */
        private DoubleRangeModel threshhold = new DoubleRangeModel(1, 0, 1000, .1);
        /** Determines relative importance of values near the goal with those far from the goal. */
        private DoubleRangeModel exponent = new DoubleRangeModel(1, -5, 5, .1);
        ...
        private TaskSettings() {
            this(1.0, new Vector<Team>(), new Team(), new Team(), SEEK, Tasking.AUTO_CLOSEST, 0.0, 1.0);
        }

        public TaskSettings(double weight, Vector<Team> teams, Team owner, Team target, int type, int tasking, double threshhold, double exponent) {
            setName("Tasking");
            this.teams = teams;
        ...
        private void initModels() {
            ...
            add(new SettingsProperty("Algorithm", this.algorithm, Settings.EDIT_COMBO));
            add(new SettingsProperty("Type", this.type, Settings.EDIT_COMBO));
            add(new SettingsProperty("Weight", this.weight, Settings.EDIT_DOUBLE));
            add(new SettingsProperty("Threshold", this.threshhold, Settings.EDIT_DOUBLE));
            add(new SettingsProperty("Exponent", this.exponent, Settings.EDIT_DOUBLE));
        }
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            ...
            if (evt.getSource() == weight) {
                ...
                if (tasker != null) {
                    tasker.type = type.getIValue();
                }
                ac = "teamSetupChange";
            } else if (evt.getSource() == threshhold || evt.getSource() == exponent) {
                ac = "teamSetupChange";
